...
require_once( \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('lumogooglemaps') . 'GoogleMapAPI.class.php');

class tx_lumogooglemaps_pi1 extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
	...
	function main($content, $conf)	{
		...
		if ($this->lConf['zoom'] != 'auto') {
			// datamints: eine Zeile eingef�gt, weil sonst "auto" sonst nie deaktiviert wird.
...
#echo "<pre>";var_dump($this->oMap);; echo "</pre>";die();
		// Fill marker array
		$lMarkerArray = array();
	...
	function init($conf) {
		...
		$vTemplateFile = $vTemplateFile ? \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($vTemplateFile) :  \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::siteRelPath('lumogooglemaps') . 'templates/template_css.html'; // Set default
		...
		$vMarkerIcon = $vMarkerIcon ? \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($vMarkerIcon) : ''; // Set default
		...
		$vMarkerShadow = $vMarkerShadow ? \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName($vMarkerShadow) : ''; // Set default
	...
	function createMap() {
		...
		$sMapId = \TYPO3\CMS\Core\Utility\GeneralUtility::shortMD5('map' . time());
		$sYahooId = 'yahoo_' . \TYPO3\CMS\Core\Utility\GeneralUtility::shortMD5('yahoo' . time());
	...
	function createInfoBox($lRow) {
		...
		// Field: email
		$sEmail = $lRow['email'];
		if (\TYPO3\CMS\Core\Utility\GeneralUtility::validEmail($sEmail)) {
	...
	function createImage($sImageFile) {
		...
		$lConf = array();
        $lConf['img'] = 'IMAGE';
        $lConf['img.']['file'] = $sImageFile;
		$lConf['img.']['file.']['maxW'] = '100';
		$lConf['img.']['file.']['maxH'] = '100';
		$lConf['img.']['imageLinkWrap'] = '1';
		$lConf['img.']['imageLinkWrap.']['enable'] = '1';
		$lConf['img.']['imageLinkWrap.']['bodyTag'] = '<body bgColor="#ffffff">';
		$lConf['img.']['imageLinkWrap.']['wrap'] = '<a href="javascript:close();"> | </a>';
		$lConf['img.']['imageLinkWrap.']['width'] = '500m';
		$lConf['img.']['imageLinkWrap.']['height'] = '500';
		$lConf['img.']['imageLinkWrap.']['JSwindow'] = '1';
		$lConf['img.']['imageLinkWrap.']['JSwindow.']['newWindow'] = '1';
		$lConf['img.']['imageLinkWrap.']['JSwindow.']['expand'] = '17,20';
		
		// Generate image tag
		$sImage = $this->cObj->IMG_RESOURCE($lConf);
